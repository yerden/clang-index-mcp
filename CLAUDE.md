...
### Daemon
19. **Restart over notify** (§6.1). Don't implement `workspace/didChangeWatchedFiles`. When compdb changes, the daemon debounces (5s) and restarts clangd. The new clangd reuses on-disk shards that clangd itself persists under `<compdb-dir>/.cache/clangd/index/` (§6.2).
20. **clangd's shard directory must be on persistent storage.** Path is fixed at `<compdb-dir>/.cache/clangd/index/` (clangd has no flag to relocate it; do not try to invent one — `--background-index-path` is not a real clangd flag and the process will refuse to start). If that directory is container-ephemeral or wiped between CI runs, every restart cold-starts; the persistence policy in §6.2 then doesn't apply.
